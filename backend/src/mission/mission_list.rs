use common::mission::{APIMission, MissionInfo, MissionList};
...
fn from_db_mission(mission_type_map: &HashMap<i16, String>, mission: Mission) -> APIMission {
...
async fn get_api_mission_list(db_pool: Data<DbPool>) -> Json<APIResponse<Vec<APIMission>>> {
    ...
    let result: Vec<APIMission> = mission_list
        .into_iter()
        .map(|item| from_db_mission(&mission_type_map, item))
        .collect();
...
async fn get_mission_list(
    ...
    Json(APIResponse::from_result(result, "cannot get mission list"))
}
